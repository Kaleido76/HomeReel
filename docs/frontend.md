...
- 顶部 5 个页签（首页/视频库/搜索/重封/文件），每页签一个独立 TanStack Router 实例
...
- **布局**（全宽无留白，`TabHost` 的 `selfScrolling` + `fullBleed`）：左侧 `DriveRail` 顶天立地窄框，
  被**可拖动分隔条**分为三段——上栏自动枚举的盘符（`/api/disks`），中栏 pin 常用路径，**下栏
  【多媒体源】**（`/api/files/sources`）；右上 `Toolbar` 为**两行**：顶部整行**面包屑**（`pathSegments`
  拆段、每段可点击跳回祖先，最左「这台电脑」图标回初始态），底部操作按钮**全部为图标 + 即时 Tooltip**
  （上级、固定（已固定蓝色激活）、标记为多媒体源（激活紫）、剪切/拷贝/粘贴/重命名/删除、多媒体视图
  （激活蓝），文字说明经自定义 `Tooltip` 组件悬停即显——深色气泡、无原生 title 延迟）；右端状态文本
  显示项数与勾选数。排序控件不在工具栏。
...
- **剪贴板 + 底部工具抽屉**：剪切/拷贝为前端内存态（`clipboard`，刷新即失），工具栏状态文本不再显示
  ...
  抽屉清空关闭、复制粘贴后保留（可继续粘到别处）。`ToolDrawerShell` 为纯外壳，后续其它工具抽屉复用。
- **多媒体源**：标记/取消标记仅经工具栏按钮（激活态），取消**不删库**；源行有「重新扫描」按钮 +
