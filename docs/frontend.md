...
- 缓存管理（ADR-023）：层级化主从视图，**默认只列有缓存的系列**（底部一行「显示无缓存的系列」一次性开关）；
  概览条（总占用含 Remux 拆解/字幕/记忆/封面缩略图）+ 搜索过滤；置顶全局项仅孤儿缓存（有则置顶，无则隐藏，
  含 Remux 残留）；系列/单集行展示 Remux 归属（n 个 · 体积），**Remux 只在归属处清理**（系列/单视频），无全局一键清理；
  未归组单集默认只列有缓存的单集，底部一行「显示无缓存的单集」一次性开关（**带缩略图**，可预生成）；点击系列/单集在右侧打开
  **同宽同构**详情——标题区（单集显示缩略图）+ 徽标（字幕/Remux/记忆）+ 播放记忆行 + 批量操作行（预生成缓存 /
  清理字幕 / 清理 Remux / 清理记忆），单集无下方分集面板；系列另有逐集成员缓存面板（逐集清理，字幕整包处理）；
  危险清理用 Modal 确认，弹窗内以**结构化列表**展示待删项。宽屏主从分栏（固定高度内部滚动），窄屏单栏钻取 + 自然滚动。
