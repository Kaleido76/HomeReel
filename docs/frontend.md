...
- Logo 与「退出登录」保持原样。
- **工具页签（2026-09，替代原「重封」/「格式工厂」独立页签）**：图标 `Wrench`，root `/tools`；工具注册表
   `features/tools/tools.ts`，访问过的工具面板保持挂载，切换不丢状态，`?tool=id` 存于 URL。
   **响应式导航（2026-09 重构）**：桌面（≥`lg`）**左侧垂直工具栏 + 右侧工具面板**（`features/tools/ToolsPage.tsx`）；
   窄屏（<`lg`）左栏隐藏，改为全局导航下一行**全宽沉浸上下文行**（当前工具图标 + 名称 + 右侧箭头），点击
   **向下展开全宽抽屉**切换工具——行本身即抽屉列表中的「当前工具」项（蓝 + 加粗），抽屉内列其余工具；覆盖式
   不推内容、高度自适应（`max-h-[60vh]`，工具多时才撑满并可滚动）、**淡入淡出 + 轻微下滑动画（150ms，常驻
   DOM 以 opacity/transform 过渡实现）**；**遮罩只盖工具面板区、不含顶部全局导航**（背板锚定工具页容器）；
   选中 / 点行 / 点遮罩 / `Esc` 收起。左栏为项目通用圆角风格：按钮 `rounded-md`、激活态 `bg-blue-600 text-white`
   （呼应视频库视图选择器，非旧式无圆角硬栏）。当前含两个工具——
