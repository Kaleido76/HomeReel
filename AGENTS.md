...
| ADR-023 | 缓存管理页面 | 缓存页重构成**层级化主从视图**：概览条（总占用含 Remux 拆解）+ 可搜索系列列表（默认仅列有缓存的系列，底部一次性「显示无缓存系列」开关）+ 置顶全局项仅孤儿缓存（有则置顶/无则隐藏，含 Remux 残留）+ 未归组单集默认仅列有缓存的单集（带缩略图、可预生成，底部「显示无缓存的单集」开关）+ 系列详情钻取（成员缓存状态、系列级批量操作、逐集清理，字幕轨**整包处理**不逐轨删除）；**Remux 缓存按视频归属**（`remuxes` 数组）显示在系列/单集上并**仅在归属处清理**（系列级/单视频，无全局一键清理）；播放记忆并入系列维度；单集详情与系列同宽同构（无分集面板）；宽屏主从分栏 / 窄屏单栏钻取 + 自然滚动；危险清理用 Modal 确认且以**结构化列表**展示待删项。新增 `pregen` Job（预提取内封文本字幕，收口 `streaming.HandlePregen`，payload 携带 enqueue 时解析的 `{video_id,path}`）、系列级清字幕/清 Remux `DELETE /api/cache/series/{id}/subtitles|remux` 与单视频清 Remux `DELETE /api/cache/remux/{videoId}` | 旧页全量平铺随库增长无限长、逐项离散无层次；真实操作是「以系列为单位」的清理/预生成，且只做子集（如看完某剧清理其全部字幕）；Remux 是最大的可重建空间占用，需要可见、可归属到剧集/单集并在归属处清理；预生成仅面向文本字幕——封面缩略图扫描已覆盖、remux 按需生成，不做全量 explore 面板 |
