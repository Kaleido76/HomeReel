...
> 参与本项目前，请先完整阅读本文档；领域细节按需阅读 `docs/` 二级文档（见 §2）。
...
| ADR-006 | 播放策略 | **纯 Range 直连（2026-08 修订，HLS 转码层已移除）**：前端把 probe 元数据映射成 MIME/codecs 串，用 `canPlayType()` 核对目标机能力；可直连 → HTTP Range；不可直连 → 播放按钮禁用并引导格式工厂转换。判定机制运行期固化（前后端协同），不逐格式堆硬编码布尔 | 原「能力探测三层 + HLS 按需转码」在家庭场景体验差（Live 需等转码）；浏览器原生容器集 + Chromium 可解编码足以覆盖主流格式，HEVC 等按目标机能力动态核对 |
...
| ADR-015 | 系列组织 | **2026-08 管理面定稿**：系列是**用户显式创建**的管理容器，绑定根目录（`seasons.root_path`），成员 = 根目录**直接一级子文件**按文件名序 1..N（无季号结构）；**扫描只维护既有系列成员、绝不自动建系列**；单集是唯一基本实体、必须归属媒体源；**无电影/tv 结构类型**（区分走标签）；只增不拆、识别错了由用户手动归组 | 剧集/番剧是常见形态；严格「路径 + FileID」对应避免误组；系列是纯 DB 概念不随磁盘自动漂移，避免自动规则误判后难以纠错 |
